...
interface USMapProps {
  onStateSelect: (stateData: StatePathData, event: React.MouseEvent<SVGPathElement>) => void;
...
export const statePaths: StatePathData[] = [
  {
  ...
  },
  {
    ...
    "centerY": "",
    "chapterCity": "Atlanta",
    "chapterName": "Atlanta Chapter",
    "chapterDesc": "Empowering Atlanta communities with essential money management skills.",
    "className": "state-path chapter-active"
  },
  {
  ...
  },
  {
    ...
    "chapterDesc": "Providing accessible financial literacy resources and mentorship programs to the Windy City.",
    "className": "state-path chapter-active"
  },
  {
  ...
  },
  {
    ...
    "centerY": "",
    "chapterCity": "",
    "chapterName": "New Jersey Chapter",
    "chapterDesc": "Bringing financial education to communities across the Garden State.",
    "className": "state-path chapter-active"
  },
  {
  ...
  },
  {
    ...
    "centerY": "440",
    "chapterCity": "Houston",
    "chapterName": "Houston Chapter",
    "chapterDesc": "Serving the greater Houston metropolitan area with financial literacy workshops and community events.",
    "className": "state-path chapter-active"
  },
  {
...
//  normal    #cdd9c8  (soft sage)   #2e3d2b  (dark forest)
//  hover     #9ab490  (medium sage) #4a6b44  (medium forest)
//  selected  #5a8c50  (deeper sage) #7ab86a  (bright mint)
//  chapter   #2b4224  (brand dark)  #8eba7e  (mint)
//  ch.hover  #1e3019  (darkest)     #b0d8a4  (light mint)
//  ch.sel    #142210  (near-black)  #d4eece  (palest mint)
// ───────────────────────────────────────────────────────────────────────────
...
export default function USMap({ onStateSelect, selectedStateCode, onHQSelect, activeChapterCodes, isHQSelected }: USMapProps) {
  ...
  function stateFill(code: string, hasChapter: boolean, isSelected: boolean, isHovered: boolean): string {
    if (!code) return isDark ? '#253322' : '#c8e8d4'; // sub-paths: faint, non-interactive
    if (hasChapter) {
      if (isSelected) return C.chSelected;
      if (isHovered)  return C.chHover;
      return C.chapter;
    }
  ...
  function stateGlow(code: string, hasChapter: boolean, isHovered: boolean, isSelected: boolean): string {
    if (!code || (!isHovered && !isSelected)) return 'none';
    const color = hasChapter
      ? (isDark ? 'rgba(142,186,126,0.70)' : 'rgba(30,74,36,0.55)')
      : (isDark ? 'rgba(122,184,106,0.55)' : 'rgba(82,184,122,0.45)');
    return `drop-shadow(0 0 5px ${color}) drop-shadow(0 0 10px ${color})`;
  }

  const hqPulse = isDark ? 'rgba(142,186,126,0.25)' : 'rgba(30,74,36,0.18)';
  const hqFill  = isDark ? '#8eba7e' : '#1e4a24';
  const hqText  = isDark ? '#1a1f1a' : '#f7f3eb';
    ...
    >
      <g>
        {statePaths.map((state, idx) => {
          // Empty-code paths are decorative border lines — render them transparently
          if (!state.code) {
            return (
              <path
                key={idx}
          ...
          const hasChapter = !!(activeChapterCodes?.includes(state.code));
          ...
          return (
            <path
              key={state.code}
              d={state.d}
              fill={stateFill(state.code, hasChapter, isSelected, isHovered)}
              stroke={C.stroke}
              strokeWidth={0.75}
              style={{
                cursor: 'pointer',
                transition: 'fill 0.15s ease',
                filter: stateGlow(state.code, hasChapter, isHovered, isSelected),
              }}
              onClick={(e) => onStateSelect(state, e)}
        ...
        {/* HQ marker — Dallas, TX */}
        <motion.g
          transform="translate(498, 398)"
          style={{ cursor: 'pointer' }}
          onClick={onHQSelect}
          onMouseEnter={() => setHovered('__hq__')}
          onMouseLeave={() => setHovered(null)}
          animate={{ scale: isHQSelected ? 1.35 : hovered === '__hq__' ? 1.15 : 1 }}
        ...
        >
          ...
          <circle
            ...
            style={{
              filter: isHQSelected
                ? `drop-shadow(0 0 10px ${hqFill}) drop-shadow(0 0 20px ${hqFill})`
                : hovered === '__hq__'
                  ? `drop-shadow(0 0 6px ${hqFill})`
                  : 'none',
          ...
          <title>Brightside HQ — Dallas, TX</title>
        </motion.g>
      </g>
    </svg>
  );
